// SkyDepth Fog by Silent
// Instead of using a probe, this uses multiple colours interpolated
// between via depth range

// Based off Distance Fade Cube Volume
// by Neitri, free of charge, free to redistribute
// from https://github.com/netri/Neitri-Unity-Shaders

Shader "Silent/SkyDepth Fog (Height)"
{
    Properties
    {
        [Header(Base)]
        _Color1("Color Tint", Color) = (0,0,0,1)
        _Color2("Color Tint", Color) = (0,0,0,1)
        _Color3("Color Tint", Color) = (0,0,0,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float colorFogLevel = (length(rayFromCamera.xyz * sceneDepth)*0.01);
                color *= lerp(lerp(_Color1, _Color2, saturate(colorFogLevel)), _Color3, saturate(colorFogLevel-1));
